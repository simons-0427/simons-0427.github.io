...
// ── ÄNDERUNG 1 & 3: IK_WLK (Wände durchgehen) und IK_SWD (Gegner besiegen) ──
...
static const Color IK_COL[] = {
    {  80,220, 80,255 },   // SPD  grün
    {  80,150,255,255 },   // SHD  blau
    { 255,140, 40,255 },   // BMB  orange
    { 180, 80,255,255 },   // WLK  lila
    { 255, 60,120,255 }    // SWD  pink/rot
};
...
#define CWLK   ((Color){180,80,255,255})   // ── ÄNDERUNG 1: wall-walk aura
#define CSWD   ((Color){255,60,120,255})   // ── ÄNDERUNG 3: sword aura
...
static void GenRound(void) {
    ...
    for (int placed=0, t=0; placed<ni && t<400; t++) {
        ...
        if (map[iy][ix]=='.') {
            // ── Alle 5 Item-Typen gleichmäßig verteilen ──
...
static void TryMove(int dx, int dy) {
    ...
    // ── ÄNDERUNG 1: WLK – Wandkollision nur wenn pwlk inaktiv ───────────────
    if (tg=='#') {
        if (pwlk <= 0) return;
        // Durch Wand hindurchgehen – keine Tile-Effekte, direkt bewegen
        px = nx; py = ny;
        Burst(WX(px), WY(py), CWLK, 4, 2);
        return;
    }

    // ── ÄNDERUNG 2: Schild schützt auch gegen Gegner ─────────────────────────
    if (eneon) {
        for (int i = 0; i < MAX_ENE; i++) {
            Ene *e = &enes[i]; if (!e->on) continue;
            if ((int)(e->x+0.5f)==nx && (int)(e->y+0.5f)==ny) {
                if (pshield) {
                    // Schild absorbiert Treffer → Gegner stirbt
                    pshield = false;
                    e->on   = false;
                    px = nx; py = ny;
...
static void UpdEnes(float dt) {
    ...
    for (int i = 0; i < MAX_ENE; i++) {
        ...
        // ── ÄNDERUNG 2: Gegner tritt auf Spieler → Schild prüfen ─────────────
...
static void DrawMenu(float t) {
    ...
    int pw=640, ph=460, pox=SW/2-pw/2, poy=SH/2-ph/2;
    ...
    // ── ÄNDERUNG 1 & 3: aktualisierte Item-Legende ───────────────────────────
    const char *ileg[] = {
        "SPD - 2x speed 8s",
        "SHD - block trap/enemy",   // ÄNDERUNG 2
        "BMB - blast 5x5 traps",
        "WLK - ghost 6s (walls)",   // ÄNDERUNG 1
        "SWD - kill nearby foes"    // ÄNDERUNG 3
    };
    Color iicol[] = {
        IK_COL[IK_SPD], IK_COL[IK_SHD], IK_COL[IK_BMB],
        IK_COL[IK_WLK], IK_COL[IK_SWD]
    };
    int ilx = pox+360;
    for (int i=0;i<5;i++) {
        DrawRectangle(ilx-4, ly+i*24, 4, 18, iicol[i]);
    ...
    DrawText("v1.1", pox+pw-44, poy+ph-22, 13, (Color){55,50,75,180});
}
    ...
    while (!WindowShouldClose()) {
        ...
        if (gs == GS_MENU) {
            ...
            { int cx = px*CELL + CELL/2, cy = py*CELL + CELL/2;
              ...
              // ── ÄNDERUNG 1: Geist-Aura bei aktivem WLK ───────────────────
              if (pwlk > 0) {
