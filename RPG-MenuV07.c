...
{
    DrawRectangle(0,110,192,490,C_PANEL);                                        // CHANGED: width 216->192
    DrawLine(192,110,192,600,C_BORDER);                                          // CHANGED: x 216->192
    for(int i=0;i<TAB_COUNT;i++){
        int ty=118+i*46;
        bool active=(i==current_tab);
        bool hov=CheckCollisionPointRec(GetMousePosition(),(Rectangle){0,(float)ty,192,38}); // CHANGED: 216->192
        Color bg  = active?C_PANEL3:hov?C_PANEL2:C_PANEL;
        Color txt = active?C_GOLD_L:C_TEXT;
        DrawRectangle(0,ty,192,38,bg);                                           // CHANGED: 216->192
        if(active){
            DrawRectangle(0,ty,4,38,C_GOLD);
            if(edit_mode||player_edit_mode) DrawRectangle(188,ty,4,38,C_GOLD);  // CHANGED: 212->188
        }
...
static void DrawHeader(int level, int xp, int cp, int tnl_cur, bool player_edit_mode, int sel_ps)
{
    ...
    DrawText("VAELEN",22,14,28,C_GOLD_L);
    DrawText("\"S_Lord\"",22,46,14,C_MUTED);
...
int main(void)
{
    ...
    InitWindow(900,600,"Aetheria — Character Menu");
    SetExitKey(KEY_NULL);
    SetTargetFPS(60);
